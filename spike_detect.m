function [spikes,thr,index] = spike_detect(x, par ,thr)
...
thrmax = stdmax * noise_std_sorted;     %thrmax for artifact removal is based on sorted settings.e
    ...
    case 'both'
        ...
        for i=1:length(xaux)
            if xaux(i) >= xaux0 + ref
                ...
                %[aux_unused, iaux]=max(abs(xf(xaux(i):xaux(i)+63)));
                if iaux == 1 || iaux >20
                    continue
                end
                nspk = nspk + 1;
